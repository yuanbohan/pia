...
func TestWriteAllowsInternalAncestorSymlink(t *testing.T) {
	t.Parallel()
	...
	if _, err := write.Execute(context.Background(), json.RawMessage(`{"path":"alias/file.txt","content":"through ancestor"}`)); err != nil {
	...
	if got, want := string(written), "through ancestor"; got != want {
		t.Fatalf("written content = %q, want %q", got, want)
	}
}

func TestWriteRejectsFinalSymlinksAndOtherNonRegularTargets(t *testing.T) {
	t.Parallel()
	...
	for _, test := range []struct {
		...
		{path: "outside-link", want: "symbolic link"},
		{path: "dangling-link", want: "symbolic link"},
		{path: "outside-parent/new.txt", want: "workspace"},
	} {
		t.Run(test.path, func(t *testing.T) {
			got, err := write.Execute(context.Background(), json.RawMessage(fmt.Sprintf(`{"path":%q,"content":"replacement"}`, test.path)))
...
func TestWriteRootBoundarySurvivesFinalSymlinkSwap(t *testing.T) {
	parent := t.TempDir()
	rootPath := filepath.Join(parent, "workspace")
	...
	for range 300 {
		_, _ = write.Execute(context.Background(), json.RawMessage(`{"path":"swap.txt","content":"written inside"}`))
	}
	close(stop)
	<-done
...
func TestWritePinsAncestorDuringSymlinkSwap(t *testing.T) {
	parent := t.TempDir()
	rootPath := filepath.Join(parent, "workspace")
	...
	outsideTarget := filepath.Join(outsidePath, "written.txt")
	...
	for range 300 {
		_, _ = write.Execute(context.Background(), json.RawMessage(`{"path":"alias/written.txt","content":"written inside"}`))
	}
	close(stop)
	<-done
