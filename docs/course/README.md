...
- `Pia Daemon`：后续负责 acknowledged Submissions、Sessions、并发、路由与 client protocol 的长期服务；不属于第一阶段 Runtime。
...
Lesson 13 先完成一次 ownership refactor，再建立 lifecycle：删除 product-level `coding.Run` 与 coding-owned `conversation` controller，让 Session 独占 Workspace、History、projection 和 active/cancel/close，把 `internal/agent` 收敛为 run-local execution engine，并由当前 one-shot host 直接构造 Session、Advance 一次、最后 Close。它保留已有 observable behavior，不保留旧函数、类型或重复 state；Lesson 16 已删除后来被证明属于 future server/client 层的 Follow-up 与 public Wait。
...
`internal/` 是当前的有意选择：接口会随学习不断校正。`cmd/pia` 把启动时的当前目录作为 workspace，只接收一条 task prompt，用于本地运行与验收；Pia 产品名已经确定，但当前 CLI 参数与外部协议仍不稳定。等单 Session persistence/resume 稳定并出现真实 client 后，再设计 Pia Daemon、common Client Protocol 与是否需要公共 Go SDK。
