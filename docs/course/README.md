...
Lessons 11–14 已经完成实现、验证、理解确认和提交。Lesson 14 根据冻结 Pi、当前 OpenCode/Codex CLI 与 Pia ownership/lifecycle 证据建立了 Follow-up 的 Run、admission、异常 hand-back、event 与 quiescence 契约。下面七行是基于当前源码证据、Pia 代码边界和长期策略形成的**滚动假设**，不是七份已经定型的 implementation spec。Lessons 12–14 是最近且边界已足够清楚的稳定全局课次；其余行只有阶段内顺序，开课时必须重新阅读对应源码与测试、追踪当时 Pia 路径，并允许拆分、合并、换序或推翻。
...
- 当前已固定 Lessons 12–14 的全局课号；后面四行虽然顺序已确认，仍要等最近课程证据稳定后逐一编号。
