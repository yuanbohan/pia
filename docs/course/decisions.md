...
- 最小交互终端：第三阶段增加一个 non-full-screen local terminal host，消费 semantic events 并调用 Session controls，不把 UI 状态下沉给 Core Agent、Conversation 或 journal。Idle input 启动普通 user advance；active `Enter` 优先表示 same-Run steering，但 terminal 在 Runtime 当前不能接入原 Run 时仍保留该输入，并在当前 settlement 后自动路由到后续工作，不向用户拒绝或要求重输；`Tab` 表示当前 execution 结束后的 follow-up；`Esc` 表示用户停止当前 execution 并重新取得输入控制：请求取消、等待 settlement、把尚未消费的 queued Steering 与 Follow-up 移回 composer、保留 Session 且不自动启动后续 Run；`/exit` 在 active 时请求取消、等待既有 settlement 与 commit section 收敛、丢弃未消费的 steering/follow-up，再关闭 Session。Ctrl-C/Ctrl-D 不作为首版主产品控制；操作系统 signal/caller cancellation 仍可作为外部执行边界。
...
### D106. Steering 在同一 Run 安全点批量进入下一次 Provider request

- 日期：2026-07-29
- 源码与产品校准：冻结 Pi `dcfe36c79702ec240b146c45f167ab75ecddd205` 默认每个 safe boundary 只消费最老一条 Steering，但其 `PendingMessageQueue` 同时支持显式 `all` mode；当前 Codex CLI `0fb559f0f6e231a88ac02ea002d3ecd248e2b515` 的 Core 在下一 model step 前一次取走当时全部 pending input。学习者根据 Codex CLI 的真实交互经验指出，同一安全点前已经到达的多条修正应在模型产生下一次决策和工具副作用前共同可见。Pia 因此不复制 Pi 的默认 queue mode，采用两份源码共同支持的 batch-all 行为。
- Run 与消息边界：Steering 不修改 in-flight Provider request、不抢占 tool，也不启动新的 Engine Run。每个正常 assistant turn 完整形成，且该 assistant message 的全部 tool calls 都各有 terminal result 并按模型 source order 追加后，Engine 从 Session 借用当前 pending batch；每条 Steering 按 admission order 成为独立 Conversation user Message，然后由完整 Working Context 构造一次下一 Provider request。assistant 无 tools、Run 原本将停止时也经过同一 Steering decision boundary。
- Batch cutoff：Session 在一个 mutex 临界区内 detach 该切点前已经接受的全部 pending Steering。切点后到达的输入留待同一 Run 的下一正常 turn boundary，不加入已经构造或 in-flight 的 Provider request。Follow-up 保持 D102–D105 的 FIFO one-at-a-time 新 Run 语义，不因 Steering batching 改变。
- Terminal 与 Runtime 分层：普通 terminal 应持续接受编辑与回车；在 terminal 仍开放的正常路径上，非空 submission 必须被保留并自动送入当前或后续 execution，不能因 compaction、Run 间隙或已封口而向用户拒绝或要求重输。只有 `Session.Steer` 的成功 acknowledgement 才把 ownership 转给当前 Engine invocation；same-Run routing 暂不可用时，terminal coordinator 继续持有输入并在 settlement 后自动投递，而不是让 Runtime 把它谎称为原 Run 的 Steering。
- Provider failure：冻结 Pi、当前 Codex CLI 与当前 OpenCode 都在各自 retry/error settlement 后保留并继续推进 queued input，但三者的 retry mechanism 与 Run/Turn shape 不同。Pia 保持现有 Engine contract：Provider/stream failure 提交 terminal assistant 后以 non-nil error 结束当前 Engine Run，不在失败 Run 内继续消费 Steering，也不在本课增加 D84 已延后的 automatic Provider retry。Session seal 并明确交还尚未成为 Conversation Messages 的 pending input；未来 terminal coordinator 展示失败后自动把 Steering 路由为新的 execution，保持独立 user messages、admission order 与一次 first Provider decision。新 execution 接受这些 user messages 后即视为 consumed；即使其 Provider 再失败，也只保留在 History，不重复投递。Follow-up 表示“当前工作结束后再做”，而 Provider failure 说明当前工作未正常完成，因此未消费 Follow-up 不自动执行，改为恢复到 composer 等待用户编辑或重新提交。
- Runtime surface：Lesson 15 保持窄的 same-Run `Session.Steer` admission 与明确 hand-back，不把“当前或后续 execution 一定接收”的通用 submission coordinator、后台 scheduler 或 terminal-owned input queue 下沉进 Session。future terminal 负责吸收 admission unavailable 与异常 hand-back，并实现用户可见的自动路由。
- Cancel：`Esc`/`Session.Cancel` 表示用户停止当前 execution 并重新取得输入控制，而不是“中断旧 Run 后立即应用 Steering”。当前 Engine Run cancellation settlement 后，Session 明确 hand back 所有尚未成为 Conversation Messages 的 queued Steering 与 Follow-up；future terminal 把原文本移回 composer，不自动重投或启动下一 Run。恢复采用简单 intent grouping：Steering 保持自身 FIFO 并放在上部，Follow-up 保持自身 FIFO 并放在下部，既有未提交 draft 最后；不保留两类 input 的交错 admission chronology。已经消费并进入 History 的 Steering 或 Follow-up 不回滚。这个显式用户操作暂停自动消费但不删除输入，不是 Runtime rejection 或 silent loss。Provider failure 仍由 terminal 自动路由 hand-back；Cancel 则恢复到 composer，不能把两项产品 policy 隐藏成 Runtime 无条件清 queue。
- Result hand-back：`AdvanceResult` 保留 D104 的 `UnconsumedFollowUps []string`，并增加平行的 `UnconsumedSteering []string`；两个字段分别保持自身 FIFO。由于 `Esc` composer restore 已选择 Steering 在上、Follow-up 在下而非跨 intent chronology，首版不增加统一 pending-input result、kind enum 或 global sequence。只有尚未成为 Conversation user Message 的 input 才会 hand back；已经消费的 input 只留在 History，不重复返回。
- Admission API：Session 增加非阻塞 `Steer(input string) error` 与 `ErrSteerUnavailable`。它先执行与 `FollowUp` 相同的 blank-input validation；closing/closed 返回 `ErrSessionClosed`，idle、compaction、Run 间隙、error/cancellation settlement 或 final seal 等没有唯一 current Engine invocation 可接管输入的状态返回 `ErrSteerUnavailable`。当前 Provider/tool work 所属 invocation window 开放时，把克隆文本追加到 Session-owned Steering FIFO并返回 `nil`；`nil` 只确认 ownership transfer，不等待消费或 completion。admission 与 drain-or-seal 使用同一个 Session mutex 线性化；rejection 不 mutation，caller 保留文本。首版不增加 context、blocking variant 或 per-input handle。
- Close/process hand-back：pending Steering/Follow-up 只随 active `AdvanceResult` 返回给负责执行 Advance 的 caller；`Close(ctx)` 继续只改变 lifetime、请求 cancellation、等待 settlement 与关闭资源，不增加第二份 queue result。TUI `Esc` 从 Advance result 恢复 composer；`/exit` 等待 settlement 后明确忽略 hand-back 再完成 close；caller context cancellation 若能正常返回仍 hand back；Close wait timeout 不转移 queue ownership，active Advance 最终仍返回给原 caller；hard kill 无法完成内存 hand-back，等待 future journal/checkpoint 的有限恢复。
- Events：Lesson 15 不增加 queue Semantic Events。Steering admission 由 `Steer()` return 表达；consumption 复用同一 Run 内现有 user Message event；Run/Advance error/cancellation 复用既有 settlement events；异常 hand-back 由 `AdvanceResult` 表达；`Esc` composer restore 属于 future terminal 本地 UI 操作。pending preview 等真实 consumer 出现后再评估只读 projection，不根据零散 events 维护第二份 authoritative queue。
- Overflow recovery：第一次明确可恢复的 context overflow 尚未 settlement outer Advance。Session 在 failed Engine Run 后关闭旧 window，但继续持有已 accepted、尚未 consumed 的 Steering；compaction 期间不开放新 Runtime admission，成功后把该 batch 转移给 recovery `Engine.Continue`，在其第一次 Provider request 前按 admission order 追加为独立 user Messages。Follow-up 仍等整个 input recovery 成功后再 drain。compaction、Working Context derivation、Continue、第二次 overflow 或 cancellation 最终失败时，才通过 `AdvanceResult.UnconsumedSteering` hand back。successful bounded overflow recovery 是跨 Engine invocation 保留 Steering 的唯一当前例外。
- Hand-back definition：hand-back 表示 pending input 的唯一 ownership 从 Session 转回负责执行 Advance 的 caller。返回的文本尚未成为 Conversation Message、不在 History 中，Session 已从 queue 移除且不得稍后执行；future terminal 再按 settlement 原因自动重投、恢复 composer 或在 `/exit` 时明确忽略。已经 consumed 的 input 只留在 History，不得同时出现在 hand-back result。
- Queue ownership 与同步：Steering queue 直接属于 active Advance control，并只在现有 Session mutex 下访问；不增加第二把锁、buffered channel、专用 coordinator goroutine 或 atomic/CAS queue。Session 是共享 slice 的唯一 mutable owner；Engine 不接触该 slice，只通过借用的 invocation-local control 请求原子 drain/drain-or-seal，并取得 detached ownership-independent batch。这个选择既保留 concurrent `Steer` admission 与 final seal 的线性化，也避免为低频输入增加 channel capacity、goroutine lifetime、acknowledgement 与 close protocol。
- Engine control：`internal/agent` 以 consumer-owned `SteeringSource` 定义 `Drain() []string` 与 `DrainOrSeal() []string`；前者 detach 当前 batch 并保持 admission 开放，后者返回当前 final available batch，或在 invocation 应停止时原子 seal 并返回 empty。`internal/coding` 的 invocation-local adapter 引用 Session 与当前 active Advance 并在 Session mutex 下实现操作；Engine 只消费返回的独立 batch，不依赖 Session、queue、lock 或 hand-back。两个具名操作比 `Drain(sealIfEmpty bool)` 更直接表达两个 loop boundary。
- Invocation wiring：`Engine.Run` 与 `Engine.Continue` 都显式要求 non-nil invocation-local `SteeringSource`；它不进入保存稳定依赖的 `agent.Config`。production Session 总是提供 control，Agent 独立测试使用 test-local empty implementation；不以 optional nil、variadic 参数、context value、Engine default 或公开 no-op 保留旧调用形状。
- Consumption 与 cancellation：Engine 在 Run/Continue 第一次 Provider call 前、完整 tool turn settlement 后调用 `Drain`，在 assistant 无 tools且 Run 原本将停止时调用 `DrainOrSeal`。start/post-tool 边界先检查 cancellation，detach 后必须完整追加整个 batch 为独立 user Messages，再次检查 cancellation，不能部分追加。would-stop 保留既有 terminal-wins contract：完整 `Done` terminal 已形成后直接由 Session source 在 mutex 内决定正常 drain，或因 invocation stopping而 seal、保留 pending并返回 empty；empty 不把已完成 Engine Run改写为 cancellation，outer Session 在存在 pending input时再以 cancellation hand back。drain 先 detach 则整个 batch consumed，即使 cancellation 随后阻止下一 Provider call也只留在 History。Semantic Events 保持 initial/assistant/tool-result与当前 Turn settled先于 Steering Messages，全部 Steering Messages先于下一 Turn started。
- Window lifecycle：active Advance 只增加 `acceptingSteering bool` 与 `pendingSteering []string`，不增加 window enum 或 generation token。pre-Run、compaction 与 Run gap 关闭；即将同步调用 Engine 时开放；normal final seal 或 ordinary settlement 后关闭。recoverable overflow 关闭 admission但保留同一 queue，compaction 成功后在 Continue 前重开并由 start drain 消费；最终 recovery failure 才 hand back。一个 Session 至多有一个同步 Engine invocation，tool work 在返回前全部 settlement且 adapter 不逃逸，因此当前不存在 stale invocation并发；未来若改变这些契约再增加 identity。
- Abnormal detach：每次 Engine return 先关闭 Steering admission但保留 queue；recoverable overflow 可以跨 compaction 重开。ordinary Provider/tool/pre-Run failure、cancellation、compaction/recovery failure与第二次 overflow最终通过一个 Session-mutex-held helper同时关闭两类 admission，并分别 detach `UnconsumedSteering` 与 `UnconsumedFollowUps`；尚未被 Engine acceptance 的 current Follow-up仍排在 Follow-up hand-back首位。统一的是 ownership transfer，不合并两类 FIFO。
- Cancel linearization：active Advance 保存 execution context。`Steer`、`FollowUp` 与 source drain在 Session mutex内检查 admission与 context cause；`Cancel` 和 active `Close` 在同一 mutex临界区先关闭两类 admission并调用 active `CancelCauseFunc`，再释放锁。因此 drain先取得锁则整个 batch consumed，Cancel/Close先取得锁则其返回前 cause与封口均可见、queue保留待 hand back。caller context cancellation由 source在锁内观察；不增加 stopping enum、第二把锁或 condition channel。
- Implementation：`SteeringSource` 位于 `internal/agent`；run-local execution在 start/post-tool/would-stop消费，coding adapter与 admission/window操作位于 `internal/coding/steering.go`，active queue state仍属于 `activeAdvance`。验证覆盖 batch/request/event ordering、final seal race、Provider error、drain/cancel whole-batch ownership、admission states、双 queue hand-back、overflow transfer、full check与race。Lesson 15不增加 queue events、generic pending record、terminal、persistence、Provider retry或multi-Session coordination。

### D107. 严格 C/S 下 future Submission 外移，Session 只保留 current-execution Steering

- 日期：2026-07-30
- 产品拓扑：Pia 的长期交互采用严格 Client/Server 架构。TUI、GUI、Mobile 与 IM Gateway 都是 Pia Daemon 的 clients，并通过同一套未来协议接入；当前 `cmd/pia` one-shot 只保留为本地开发与验收入口，不代表长期 interactive integration。Client 拥有 draft 与尚未被 server acknowledgement 接收的输入；Daemon service layer 未来拥有已确认但尚未开始的 Submissions、routing、result delivery 与 multi-Session coordination；Session 不直接暴露给 interactive clients。
- 取代的 topology：本决定取代 D52 中“Gateway/IM 与 direct local interactive topology 仍开放”的部分，也取代 D84 与第三阶段旧路线中“最小 terminal 直接作为 Session host/coordinator”的判断。Gateway 是 IM 到 Pia Protocol 的 client adapter，不是 Session owner；client disconnect、Terminal `/exit`、Daemon shutdown 与 CloseSession 是不同 scope，未来协议必须分别定义，不能由 Session 根据 transport 断开自行推断。
- Session ownership：一个 Session 继续唯一拥有一个 Conversation 的 Workspace/resources、完整 History、compaction/recovery projection、lifetime、至多一个 active Advance，以及已经被当前 Engine invocation 接受的 Steering。Session 不拥有 acknowledged future Submissions、client draft、submission identity、durable inbox 或 routing policy。一个 external Submission 只启动一个 Advance；该 Advance 仍可以包含 pre-Run compaction、同一 Run 的 accepted Steering 和 eligible overflow 后的 input-free continuation，但不再排空多个 Follow-up Runs。
- Future routing：未来 Daemon 在 Session idle 时以 Submission 启动新的 synchronous Advance；Session active 时可以尝试把 ordinary Submission 交给 current execution；若当前 invocation 不可接入，Daemon 保持 ownership 并在 settlement 后启动新的 Advance。这个 routing miss 是正常状态，不应成为 client-visible rejection。显式 Follow-up 可以保留为某个 client 的 delivery intent，例如 Terminal Tab 表示“不要尝试当前 Steering”，但它不再是 Session API、queue 或 lifetime state；是否进入 common protocol 等待真实 clients 校准。
- Application surface：Lesson 16 的目标 surface 为 `NewSession(SessionConfig)`、immutable `Info`、同步 `Advance(ctx, string) (AdvanceResult, error)`、`TrySteer(input string) (bool, error)`、`Cancel()` 与 `Close(ctx)`。`TrySteer` 的 `true, nil` 表示 ownership 已转移给当前 invocation；`false, nil` 表示没有当前 steerable window且 caller 仍拥有文本；blank input 与 closing/closed 等真实失败继续返回 error。删除 `Steer`/`ErrSteerUnavailable`，因为预期中的 routing miss 不是 error。`ErrSessionBusy` 只保留为防御错误 concurrent Advance 的 invariant，不承担 future queue routing。
- Follow-up supersession：D102–D105 记录的 Run、admission、hand-back 与 event 语义是 Lesson 14 的有效历史学习证据，但其 Session-owned capability 被本决定完整取代。Lesson 16 删除 `Session.FollowUp`、`ErrFollowUpUnavailable`、Follow-up window/queue/drain/hand-back、`AdvanceResult.UnconsumedFollowUps` 和专用 tests，不保留 compatibility shim。D106 中所有依赖双 queue、composer grouping、`/exit` queue ignore 或 provider-failure terminal auto-route 的部分也不再是 Session Runtime policy。
- Wait supersession：D99/D100 的 public `Wait(ctx)` 被本决定取代。当前生产 caller 没有独立 Wait consumer；future Daemon 可以等待自己启动的 synchronous Advance，Close 已经需要且继续使用 private completion signal。Lesson 16 删除 public Wait，但保留 D98 的 Close 语义：永久停止 admission、立即取消 active work、在 caller context 内有界等待 settlement 与 resource close，timeout 不重新开放、不强关借用中的资源、不冒充 clean close。
- Cancel scope：`Cancel()` 继续 nonblocking、idempotent，只请求取消唯一 active Advance；idle 时 no-op，不关闭 Session，也不删除或重排 future Daemon-owned Submissions。Client `Esc` 以后可以通过协议映射到 active cancellation，但 `Esc`、Tab、Enter 等 UI action 不是 Session Runtime control 名称。
- Steering owner 与 hand-back：D106 的 batch-all、start/post-tool/would-stop safe points、overflow transfer 与 accepted-but-unconsumed hand-back 继续有效。Session 只保存 `TrySteer` 已接受的 current-invocation Steering；accepted 但未被 Engine drain 的文本仍只通过 `AdvanceResult.UnconsumedSteering` 唯一交回 Advance caller。Session 不按 Provider failure、Cancel、Close 或 caller cancellation 自动 requeue、drop 或恢复 client composer；已成为 Conversation user Message 的 consumed Steering 只存在于 History，不得重复 hand back。
- Engine/Session handshake：Engine 继续拥有一个完整 run-local Provider/tool loop、tool settlement、terminal closure、Run ordering 与 delta；不缩成单次 Provider call，也不由 Session step-drive。Engine 识别第一次 Provider request 前、完整 tool turn 后和 would-stop 的 safe boundary；Session 在同一 mutex 下同步返回已接受 batch，或在 would-stop 时原子确认 empty 并 seal invocation。当前 `SteeringSource` 与 `Drain`/`DrainOrSeal` 是可在实现中评估的 Go mechanism，不是耐久 API 名；无论具体命名如何，不得改成第二套 goroutine/channel lifecycle。
- Linearization：Steering drain、would-stop seal、Cancel 与 active Close 继续使用同一 Session mutex。Drain 先取得 batch时，整个 batch consumed 并只进入 Engine/History；Cancel/Close 先封口时，pending batch 不进入 Engine并在 Advance settlement hand back。这个 exactly-one 语义不得因删除 Follow-up 或重命名 admission 而削弱。
- 实现结果：Lesson 16 已删除 Follow-up error/API/result field、future queue、public `Wait` 与 per-Advance completion channel；`Advance` 只执行一个 external initial input，`TrySteer` 使用 bool 表达 ownership transfer。现有 `SteeringSource`、`Drain`/`DrainOrSeal` 与 Engine loop 保持不变；cancel/close hand-back、successful terminal 后的 pending hand-back、overflow transfer 与 final-seal race 已通过 focused tests、`make check` 和 `go test -race ./...`。
...
- 2026-07-29：Lesson 15 开课对照冻结 Pi、当前 Codex CLI 与 OpenCode 后，学习者确认 terminal input capture 与 Runtime Steering admission 分层，并选择 D106 的 batch-all 语义：同一 safe boundary 前已接受的全部 Steering 保持为独立 user Messages，用一次下一 Provider request 处理；这取代此前沿用 Pi 默认 `one-at-a-time` 的课程假设，Follow-up 的逐条新 Run 语义不变。
- 2026-07-29：学习者进一步明确 terminal 产品要求：正常 submission 必须被保留并自动消费，不能把 same-Run Steering 已封口表现为用户可见 rejection。D84 与 D106 因此区分“当前 Run 能否接入”与“产品是否接受输入”：前者仍需原子边界，后者在 terminal 开放期间始终接受，并由 coordinator 在原 Run 不可接入时自动路由到后续工作。
- 2026-07-30：对照冻结 Pi、当前 Codex CLI 与 OpenCode 的 Provider failure 路径后，学习者确认 Lesson 15 采用“停失败 Run，不停输入队列”：Pia 保持 Provider failure 以 non-nil error 结束当前 Engine Run，pending Steering 明确 hand back；未来 terminal 在 settlement 后自动把该 batch 作为新的 execution 消费，不要求重输、不在失败 Run 内继续，也不提前实现 automatic Provider retry 或通用 Session scheduler。
- 2026-07-30：学习者纠正 cancellation routing：若 `Esc` 后自动启动 retained Steering，用户需要第二次 `Esc` 才能真正停下；但停止执行也不等于删除文本。D84/D106 因此固定 `Esc` 为停止当前 execution 并重新取得输入控制：取消后 hand back 尚未消费的 queued Steering 与 Follow-up，由 TUI 恢复到 composer，不自动重投、不要求重输，也不静默丢弃。
- 2026-07-30：学习者确认 Provider failure 后按 input intent 分流 hand-back：pending Steering 自动路由为新的 execution，未消费 Follow-up 因原工作未正常完成而恢复到 composer，不自动执行；两者都保留文本与各自顺序。
- 2026-07-30：学习者选择简单的 `Esc` composer restore ordering：未消费 Steering 按自身 FIFO 放上部，未消费 Follow-up 按自身 FIFO 放下部，既有 draft 最后；不为两类 input 的交错提交顺序增加统一 sequence。
- 2026-07-30：学习者确认 hand-back result 保持两个独立 FIFO 字段：新增 `AdvanceResult.UnconsumedSteering []string`，保留 `UnconsumedFollowUps []string`；不引入统一 kind/sequence 结构。
- 2026-07-30：学习者确认 Steering admission API 为非阻塞 `Session.Steer(input string) error`：`nil` 只转移 ownership，blank input 先 validation，closing/closed 返回 `ErrSessionClosed`，没有唯一可接入 Engine invocation 的状态返回 `ErrSteerUnavailable`；不增加 context、wait 或 per-input handle。
- 2026-07-30：学习者确认 Close/process hand-back 保持单一 result owner：pending Steering/Follow-up 只随 active `AdvanceResult` 返回，`Close(ctx)` 不复制 queue result；`Esc` 恢复、`/exit` 忽略、caller cancellation 的正常返回与 hard-kill limitation 由 host 分别处理。
- 2026-07-30：学习者确认 Lesson 15 不新增 queue Semantic Events；admission、consumption、settlement 与 hand-back 分别复用同步 API result、既有 Message/Run/Advance events 与 `AdvanceResult`，pending preview 留给真实 terminal consumer。
- 2026-07-30：学习者确认 successful context-overflow recovery 跨 failed Run、compaction 与 recovery `Engine.Continue` 保留 accepted Steering，并在第一次 recovery Provider request 前消费；最终 recovery failure/cancellation 才 hand back。hand-back 被明确为 Session 通过 `AdvanceResult` 把未消费文本的唯一 ownership 交回 Advance caller，而不是某一种固定 UI 动作。
- 2026-07-30：学习者委托按最佳实践直接确定 Lesson 15 剩余内容并实现。D106 已按测试先行落地：Engine 使用 invocation-local `SteeringSource` 在 start、post-tool 与 would-stop 安全点 batch drain，Session 使用单 mutex 管理 `Steer` admission、final seal、Cancel/Close 线性化、双 queue hand-back 与 overflow transfer。简化审查合并 input validation 和异常 detach 路径；结构化主线程审查补齐 tool-stage cancellation 不 drain 与 consumed Steering 不重复 hand back 的测试。最终 `make check` 和 `go test -race ./...` 全部通过，课程进入待理解确认且尚未提交。
- 2026-07-30：学习者明确要求把 Lesson 15 提交并直接推送到 `main`。课程状态更新为已提交；下一课不会因此自动开始。
- 2026-07-30：学习者明确开始 Lesson 16，并在对照冻结 Pi、Codex CLI、OpenCode 与当前 Pia 后确认 D107：长期采用严格 C/S，所有交互 clients 通过 future Pia Daemon 的 common protocol 接入；future Submissions 外移到 server layer，Session 只保留 lifetime、active Advance 与 current-execution Steering。课程确定删除 Session Follow-up 与 public Wait，把 Steer 收敛为 TrySteer，并保留 Engine 完整 loop、safe-boundary handshake、policy-neutral Steering hand-back、Cancel 与 Close；Daemon/protocol 只记录方向，不在本课实现。
